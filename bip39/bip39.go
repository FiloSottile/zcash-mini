// Partially derived from github.com/tyler-smith/go-bip39
...
func Encode(data []byte) []string {
	...
	dataInt.Lsh(dataInt, uint(bitLen/32))
	ck := sha256.Sum256(data)
	ckInt := new(big.Int).SetBytes(ck[:])
	ckInt.Rsh(ckInt, uint(256-bitLen/32))
	dataInt.Add(dataInt, ckInt)

	bitLen += bitLen / 32
	sentenceLength := bitLen / 11
...
func Decode(words []string) (data []byte, corrections []string, err error) {
	...
	bitLen := len(words) * 11
	ckLen := uint(bitLen / 33)
	dataLen := bitLen - int(ckLen)
	...
	for _, w := range words {
		...
		if !ok {
			return nil, nil, fmt.Errorf("unrecognized word: %s", w)
		}
		if w != entry.W {
			corrections = append(corrections, w+" -> "+entry.W)
		}

		wordBits := big.NewInt(int64(entry.N))
		dataInt.Lsh(dataInt, 11)
		dataInt.Or(dataInt, wordBits)
	}

	mask := big.NewInt(1)
	mask.Sub(mask.Lsh(mask, ckLen), big.NewInt(1))
	checksum := new(big.Int).And(dataInt, mask)

	dataInt.Rsh(dataInt, uint(ckLen))
	data = dataInt.Bytes()
	if len(data) < dataLen/8 {
		d := make([]byte, dataLen/8)
		copy(d[dataLen/8-len(data):], data)
		data = d
	}

	ck := sha256.Sum256(data)
	ckInt := new(big.Int).SetBytes(ck[:])
	ckInt.Rsh(ckInt, uint(256-ckLen))

	if ckInt.Cmp(checksum) != 0 {
		return nil, nil, errors.New("bad checksum")
	}
...
var WordList []string
var WordMap = make(map[string]wordEntry)

func init() {
	WordList = strings.Split(englishWordList, " ")
